...
// Variant helpers
const applyFilters = (...filters) => (attr) => filters.every((filter) => filter(attr))
export const typeFilter = (type) => (attr) => attr.type === type
export const valueFilter = (value) => (attr) => attr.value === value
export const attributeFilter = (type, value) => applyFilters(typeFilter(type), valueFilter(value))
export const variantFilter = (type, value) => ({ attributes }) => attributes.some(attributeFilter(type, value))
export const typeMapper = (type) => (value) => ({ type, value })
export const defaultMapper = (attrType) => ({ attributes }) => {
  const { type, value } = attributes.find(typeFilter(attrType))
