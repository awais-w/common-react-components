...
const getOptionsPath = (pageNumber, sortBy, searchType) => {
  ...
  const getSortBy = () => {
    ...
    const sortByObject = sortByValues.find((value) => value.key === `${key}_${sortBy[key]}`)
...
const getCommonParametersPath = (params, isBrowse = false) => {
  const { sortBy, pageNumber, searchType } = params
  let path = ''
  ...
  const optionalPath = getOptionsPath(pageNumber, sortBy, searchType)
