...
describe('orderVariants helper', () => {
  ...
  // NEEDS NODE 9+
  xdescribe('Performs well on large data', () => {
    test('it completes under 25ms for over 100 entries', async () => {
      performance.mark('orderVariants:start')
      const computed = validateType('size', lotsSizes)
      performance.mark('orderVariants:finish')
      performance.measure('orderVariants', 'orderVariants:start', 'orderVariants:finish')
      const measure = performance.getEntriesByName('orderVariants').shift()
      expect(measure.duration).toBeLessThan(25)
    })
  })
})
